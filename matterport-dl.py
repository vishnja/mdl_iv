...
MAX_CONCURRENT_REQUESTS = 4  # cffi will make sure no more than this many curl workers are used at once (reduced to avoid CDN rate-limits)
...
async def downloadFile(type, shouldExist, url, file, post_data=None, always_download=False, key_type: AccessKeyType = AccessKeyType.PrimaryKey):
    """
    Download a file with retry/backoff for HTTP 429 and transient 5xx errors.
    Keeps the original behavior for 404s (no retry) and logs retries via logUrlDownloadFinish.
    """
    ...
    async with MAX_TASKS_SEMAPHORE:
        ...
        if not CLA.getCommandLineArg(CommandLineArg.DOWNLOAD) or (os.path.exists(file) and not always_download):
            # skip already downloaded files except always_download ones
        ...
        while True:
            try:
                ...
                # Handle explicit 429 with Retry-After header if provided
                if status == 429:
                    retry_after = None
                    try:
                    ...
                    if attempt < max_retries:
                        ...
                        err_msg = f"HTTP 429: Too Many Requests after {attempt} attempts"
                        logUrlDownloadFinish(type, file, url, "", shouldExist, reqId, err_msg)
                        raise Exception(err_msg)

                # If no 429, raise for other error codes and handle below
                ...
                err_str = f"{err}"
                # If 404 - do not retry
                if "Error 404" in err_str or getattr(err, "status_code", None) == 404:
                ...
                # Exhausted retries, report and raise
                logUrlDownloadFinish(type, file, url, "", shouldExist, reqId, err)
                raise Exception(f"Request error for url: {url} ({type}) that would output to: {file} of: {err}") from err
